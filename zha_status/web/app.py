...
You are absolutely right! My apologies again. In the iterative process of making changes, the /raw_zha_data route indeed got lost. This route is very useful for debugging and inspecting the raw data that collector.py generates.

Let's put it back into app.py. It will simply load the zha_data.json file and return its content as a JSON response.

Here's the complete and corrected app.py with the /raw_zha_data route restored:

app.py (with /raw_zha_data route restored)
Python
